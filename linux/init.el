...
;; #TODO: Make separate TERM configuration (emacs -nw)
...
;; = ensure $PATH etc are passed correctly to Emacs
...
;; use EWW as default URL browser
...
(setq directory-free-space-args "-kh" ; human-readable free space, 1K block
      dired-listing-switches "-lahGg" ; don't show group, human-readable space, no owner
      dired-recursive-copies 'top      ; ask for top dir only ('always for never asking)
...
(use-package pdf-tools  ;; install separately (via package manager)
  ...
  (setq pdf-view-midnight-colors (quote ("#e6e6e6" . "#2e2e2e")))
  ;; dracula (quote ("#f8f8f2" . "#282a36")))
  ;; nord (quote ("#D8DEE9" . "#2E3440")))
...
;; === SPELLING
;; just install aspell aspell-ro aspell-en
(use-package flyspell
  ...
  (setq ispell-dictionary "ro-classic")
...
(use-package modus-themes
  ...
  (setq
   ...
   modus-themes-syntax (quote (alt-syntax))    ; faint, yellow-comments, green-strings, alt-syntax
   )
  )
