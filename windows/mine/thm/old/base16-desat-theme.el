;; base16-desat-theme.el -- base16-desat themes for emacs

;; Author: Kaleb Elwert <belak@coded.io>
;;         Neil Bhakta
;; Maintainer: Kaleb Elwert <belak@coded.io>
;; Version: 1.1
;; Homepage: https://github.com/belak/base16-desat-emacs

;;; Commentary:
;; base16-desat-theme is a collection of themes built around the base16-desat
;; concept (https://github.com/chriskempson/base16-desat).  All themes are
;; generated from the official set of color schemes and the templates
;; which are included in this repo.

;;; Code:

(defvar base16-desat-theme-use-shell-colors nil
  "Enable support for base16-desat-shell.

Because base16-desat-shell mangles the color space, we need to use a
different color translation.  This needs to be specified manually
before the themes are loaded.")

(defvar base16-desat-shell-colors
  '(:base00 "black"
    :base01 "brightgreen"
    :base02 "brightyellow"
    :base03 "brightblack"
    ...
    :base0E "magenta"
    :base0F "brightcyan")
  "Base16-Desat colors used when in a terminal and not using base16-desat-shell.
...
(defvar base16-desat-shell-colors-256
  '(:base00 "black"
    :base01 "color-18"
    :base02 "color-19"
    :base03 "brightblack"
    ...
    :base0E "magenta"
    :base0F "color-17")
  "Base16-Desat colors used when in a terminal and using base16-desat-shell.

These mappings are based on the xresources themes combined with
the base16-desat-shell code.  If you're using a different terminal
color scheme, you may want to look for an alternate theme for use
in the terminal.")

(defun base16-desat-transform-spec (spec colors)
      ...
      (let* ((key       (car  spec))
        ...
        (cond
         ((and (memq key '(:box :underline)) (listp value))
          (setq output (append output (list key (base16-desat-transform-spec value colors)))))
         (color
...
(defun base16-desat-transform-face (spec colors)
  "Transform a face `SPEC' into an Emacs theme face definition using `COLORS'."
  (let* ((face         (car spec))
         (definition   (cdr spec))
         (shell-colors (if base16-desat-theme-use-shell-colors base16-desat-shell-colors-256 base16-desat-shell-colors)))

    (list face `((((type graphic)) ,(base16-desat-transform-spec definition colors))
                 (t                ,(base16-desat-transform-spec definition shell-colors))))))

(defun base16-desat-set-faces (theme-name colors faces)
  ...
  (apply 'custom-theme-set-faces theme-name
         (mapcar #'(lambda (face)
                     (base16-desat-transform-face face colors))
                 faces)))

(defun base16-desat-theme-define (theme-name theme-colors)
  "Define the faces for a base16-desat colorscheme given a `THEME-NAME' and a plist of `THEME-COLORS'."
  (base16-desat-set-faces
   theme-name
   theme-colors
...
;;;; basic colors
     ...
     (bold                                          :foreground base06)
     (italic                                        :foreground base06)
     (bold-italic                                   :foreground base06)
...
;;;; mode-line
     (mode-line                                    :foreground base04 :background base00 :box nil)
     ...
     (mode-line-major-mode-face                    :foreground base0D)
     (mode-line-80col-face                         :foreground base08)
...
;;;; auctex
     (font-latex-bold-face                         :foreground base0C)
     (font-latex-doctex-documentation-face         :background base03)
     (font-latex-italic-face                       :foreground base0C)
     (font-latex-math-face                         :foreground base0B)
...
;;;; org-mode
     ...
     (org-block                                    :foreground base05) ;; "#afafaf")
;;     (org-code                                     :foreground base0A)
     ...
     (org-level-2                                  :foreground base0B)
     (org-level-3                                  :foreground base0C)
     (org-level-4                                  :foreground base0D)
     (org-level-5                                  :foreground base0E)
     (org-level-6                                  :foreground base0E)
     (org-level-7                                  :foreground base0E)
     (org-level-8                                  :foreground base0E)
...
(provide 'base16-desat-theme)

;;; base16-desat-theme.el ends here
